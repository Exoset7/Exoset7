...
ask_optional_setup_audio_pro () {
    ...
    2>&1 1>/dev/tty);
        if [[ $audio == "1" ]]; then
            ...
            arch-chroot /mnt bash -c "mkdir -p /etc/skel/audio-pro && cd /etc/skel/audio-pro && curl -LJO https://raw.githubusercontent.com/Exoset7/bridge-pulseaudio-jack/main/qjackctl-bridge-pulseaudio.sh"
            arch-chroot /mnt bash -c "mkdir -p /etc/skel/audio-pro && cd /etc/skel/audio-pro && curl -LJO https://raw.githubusercontent.com/Exoset7/bridge-pulseaudio-jack/main/ardour-preset.tar.gz && tar -xf ardour-preset.tar.gz && rm ardour-preset.tar.gz"
